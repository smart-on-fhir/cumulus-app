...
function buildChartOptions(transmissions: app.Transmission[], sites: app.DataSite[]): Highcharts.Options
{
    const series: {
        site      : app.DataSite,
        dataStart : number,
...
export default function TransmissionChart({ transmissions = [], sites = [] }: TransmissionChartProps)
{
    const options = buildChartOptions(transmissions, sites)
    return (
        <div>
            <Chart containerProps={{ id: "transmission-chart" }} options={options} />
        </div>
    )
}
